...
- **現在価格**: ${current:,.2f} ({change_pct:+.2f}%)"""
        ...
        with st.expander("ℹ️ 用語解説", expanded=False):
            st.markdown('<div style="font-size: 0.6rem; color: #d0d0d0; padding: 0.18rem 0.5rem; margin-top: -1.5rem;">現在価格の変動率 - プラス（+）: 前の時間帯より上昇 → 上昇トレンドの可能性が高い / マイナス（-）: 前の時間帯より下落 → 下落トレンドの可能性が高い</div>', unsafe_allow_html=True)
