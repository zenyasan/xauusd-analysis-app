...
<div style="background: linear-gradient(135deg, rgba(255, 100, 0, 0.15) 0%, rgba(255, 50, 0, 0.15) 100%); 
    ...
    <h4 style="color: #ff6600; margin-top: 5.0; font-size: 1rem;">⚠️ 重要な免責事項</h4>
    <p style="font-size: 0.8rem; line-height: 1.6; margin-bottom: 0; color: #c0d8ff;">
        ...
        <strong style="color: #00aaff;">投資助言・売買推奨ではありません</strong>。<br>
        実際の投資判断は、必ずご自身の責任と判断で行ってください。<br>
        本アプリの利用による損失について、運営者は一切の責任を負いません。<br>
        <strong style="color: #ff6600;">※金融商品取引業の登録はありません</strong>
    </p>
</div>
""", unsafe_allow_html=True)
...
def generate_advanced_analysis(style, current, change_pct, rsi, macd, macd_signal, atr, support, resistance, pivot, r1, s1, timeframe):
    ...
    """
        ...
        analysis6 = f"""
    ### 📚 テクニカル分析の教科書的な考え方（教育目的）
    
    以下は、一般的なテクニカル分析の教科書に記載されている内容を、現在の価格に当てはめた「分析例」です。
    <strong style="color: #ff6600;">投資推奨ではありません。</strong>
    
    #### 🟢 ロング（買い）を検討する場合の一般的な分析例
    
    **テクニカル分析上の注目ポイント：**
    - サポートライン（${s1:,.2f}～${pivot:,.2f}付近）での反発の可能性
    - RSIが40以下で売られすぎの状態
    - MACDが上昇転換するタイミング
    
    **一般的な損益管理の考え方（参考例）：**
    - 想定エントリー付近: ${targets['long']['entry']:,.2f}前後
    - 損切りライン候補: ${targets['long']['sl']:,.2f}前後（ATR 1.5倍を基準とした場合）
    - 利確候補1: ${targets['long']['tp1']:,.2f}前後（ATR 2倍を基準とした場合）
    - 利確候補2: ${targets['long']['tp2']:,.2f}前後（ATR 3倍を基準とした場合）
    - リスクリワード比率: 1:{rr_long:.2f}（参考値）
    
    上記は教科書的な分析例であり、<strong style="color: #ff6600;">実際の投資判断はご自身の責任で行ってください。</strong>
    
    #### 🔴 ショート（売り）を検討する場合の一般的な分析例
    
    **テクニカル分析上の注目ポイント：**
    - レジスタンスライン（${pivot:,.2f}～${r1:,.2f}付近）での反落の可能性
    - RSIが60以上で買われすぎの状態
    - MACDが下降転換するタイミング
    
    **一般的な損益管理の考え方（参考例）：**
    - 想定エントリー付近: ${targets['short']['entry']:,.2f}前後
    - 損切りライン候補: ${targets['short']['sl']:,.2f}前後（ATR 1.5倍を基準とした場合）
    - 利確候補1: ${targets['short']['tp1']:,.2f}前後（ATR 2倍を基準とした場合）
    - 利確候補2: ${targets['short']['tp2']:,.2f}前後（ATR 3倍を基準とした場合）
    - リスクリワード比率: 1:{rr_short:.2f}（参考値）
    
    上記は教科書的な分析例であり、<strong style="color: #ff6600;">実際の投資判断はご自身の責任で行ってください。</strong>
    
    ### ⚠️ 一般的なリスク管理の考え方
    
    以下は、テクニカル分析の一般的な注意点です：
    
    - **スプレッド考慮**：ブローカーのスプレッドは通常2-5ドル程度あります
    <small class="note-text">→ 価格表示と実際の約定価格には差があることを理解しましょう</small>
    
    - **経済指標発表時の注意**：重要指標の前後30分は価格が急変動します
    <small class="note-text">→ 雇用統計、GDP、FOMC発表など。Investing.comで経済カレンダーを確認できます</small>
    
    - **連続損失時の対処**：一般的に、連続3回損失が出た場合は休憩が推奨されます
    <small class="note-text">→ 感情的にならないよう、一時的に取引から離れることが大切です</small>
    
    - **ボラティリティ確認**：ATRが平均の1.5倍以上の場合、価格変動が大きいため注意が必要です
    <small class="note-text">→ 通常ATRが10-15の場合、22以上なら慎重な判断が求められます</small>
    
    <strong style="color: #ff6600;">※本情報は教育目的のみで提供されており、投資助言ではありません。</strong>
    """
        st.markdown(analysis6, unsafe_allow_html=True)
    
    elif style == "デイトレード":
        analysis = f"""
## 📊 デイトレード分析（{timeframe}）- 教育目的

### トレンド判定（一般的な分析）
{"📈 **強い上昇トレンドの可能性** - ロングを検討する分析例" if change_pct > 0.5 and macd > macd_signal else "📉 **強い下落トレンドの可能性** - ショートを検討する分析例" if change_pct < -0.5 and macd < macd_signal else "➡️ **レンジ相場の可能性** - ブレイクアウトを待つ分析例"}
...
以下は一般的なテクニカル分析の参考例です。<strong style="color: #ff6600;">投資推奨ではありません。</strong>

#### 🟢 ロング（買い）の一般的な分析例

**注目する価格帯（参考）：**
- ${s1:,.2f}〜${targets['long']['entry']:,.2f}付近
- サポートでの反発を確認する考え方

**一般的な利確候補（参考例）：**
- 第1目標候補（30%）: ${targets['long']['tp1']:,.2f}前後
- 第2目標候補（40%）: ${pivot + atr:,.2f}前後
- 第3目標候補（30%）: ${targets['long']['tp2']:,.2f}前後

**リスク管理の一般的な考え方：**
- 損切りライン候補: ${targets['long']['sl']:,.2f}前後
- 一般的な許容損失: 資金の1%以下が推奨されています
- リスクリワード比率: 1:{rr_long:.2f}（参考値）

#### 🔴 ショート（売り）の一般的な分析例

**注目する価格帯（参考）：**
- ${targets['short']['entry']:,.2f}〜${r1:,.2f}付近
- レジスタンスでの反落を確認する考え方

**一般的な利確候補（参考例）：**
- 第1目標候補（30%）: ${targets['short']['tp1']:,.2f}前後
- 第2目標候補（40%）: ${pivot - atr:,.2f}前後
- 第3目標候補（30%）: ${targets['short']['tp2']:,.2f}前後

**リスク管理の一般的な考え方：**
- 損切りライン候補: ${targets['short']['sl']:,.2f}前後
- 一般的な許容損失: 資金の1%以下が推奨されています
- リスクリワード比率: 1:{rr_short:.2f}（参考値）

### ⏰ 時間帯ごとの一般的な特徴
- **9:00-12:00（東京時間）**: トレンドフォロー型の取引が多い時間帯、ボラティリティは比較的低め
- **16:00-19:00（欧州時間）**: ブレイクアウトが発生しやすい、ボラティリティが増加する傾向
- **22:00-02:00（NY時間）**: 最も取引量が多い時間帯、価格変動が活発

### 📊 一般的な注意点
- {"RSIが買われすぎ水準、利確を検討する判断材料の一つ" if rsi > 70 else "RSIが売られすぎ水準、押し目買いを検討する判断材料の一つ" if rsi < 30 else "RSI中立圏、トレンドに従う判断が一般的"}
- ATRが{atr:.2f}なので、{"ボラティリティが高め、損切り幅の拡大も検討材料" if atr > 15 else "ボラティリティは通常範囲、標準的な分析が可能"}
- デイトレードでは、一般的にポジションは当日中に決済する考え方が基本です

<strong style="color: #ff6600;">※本情報は教育目的のみで提供されており、投資助言ではありません。実際の投資判断はご自身の責任で行ってください。</strong>
"""
        st.markdown(analysis)
    
    else:
        analysis = f"""
## 📈 スイングトレード分析（{timeframe}）- 教育目的

### 🌍 マクロ環境の一般的な見方
- **現在価格**: ${current:,.2f} ({change_pct:+.2f}%)
- **週次トレンドの見方**: {"上昇傾向の可能性" if change_pct > 1 else "下降傾向の可能性" if change_pct < -1 else "中立的な動き"}
- **RSI (7)**: {rsi:.1f}
- **MACD**: {macd_trend}

### 大局的トレンドの一般的な分析
{"🟢 **強気相場継続の可能性** - 押し目買いを検討する分析例" if change_pct > 1.0 and macd > macd_signal else "🔴 **弱気相場継続の可能性** - 戻り売りを検討する分析例" if change_pct < -1.0 and macd < macd_signal else "🟡 **調整局面の可能性** - レンジブレイクを待つ分析例"}

### 📚 中期ポジションの教科書的な考え方

以下は一般的なテクニカル分析の参考例です。<strong style="color: #ff6600;">投資推奨ではありません。</strong>

#### 🟢 ロング（買い）ポジションの一般的な分析例

**注目する価格帯（参考）：**
- ${s1:,.2f}〜${support:,.2f}付近
- 分割エントリーの一般的な考え方: 3回に分けて建玉する方法
  - 1回目（40%）: ${support:,.2f}前後
  - 2回目（30%）: ${s1:,.2f}前後
  - 3回目（30%）: ${s1 - atr:,.2f}前後

**利確候補の一般的な考え方（参考例）：**
- 第1目標候補（30%）: ${pivot + atr * 2:,.0f}前後
- 第2目標候補（40%）: ${r1:,.0f}前後
- 第3目標候補（30%）: ${r1 + atr * 2:,.0f}前後

**損切りの一般的な考え方：**
- 絶対損切りライン候補: ${targets['long']['sl']:,.0f}前後
- トレーリングストップの考え方: 価格が${pivot:,.0f}突破後、ピボット-ATRに引き上げる方法

**一般的な想定保有期間**: 3日〜2週間程度

#### 🔴 ショート（売り）ポジションの一般的な分析例

**注目する価格帯（参考）：**
- ${resistance:,.2f}〜${r1:,.2f}付近
- 分割エントリーの一般的な考え方: 3回に分けて建玉する方法
  - 1回目（40%）: ${resistance:,.2f}前後
  - 2回目（30%）: ${r1:,.2f}前後
  - 3回目（30%）: ${r1 + atr:,.2f}前後
...
- 第1目標候補（30%）: ${pivot - atr * 2:,.0f}前後
- 第2目標候補（40%）: ${s1:,.0f}前後
- 第3目標候補（30%）: ${s1 - atr * 2:,.0f}前後

**損切りの一般的な考え方：**
- 絶対損切りライン候補: ${targets['short']['sl']:,.0f}前後
- トレーリングストップの考え方: 価格が${pivot:,.0f}下抜け後、ピボット+ATRに引き下げる方法

**一般的な想定保有期間**: 3日〜2週間程度

### 🌐 ファンダメンタル要因の一般的な考え方
- 地政学リスク（中東情勢など）→ 一般的に金価格の上昇要因とされます
- FRB政策（利上げ観測など）→ 一般的に金価格の下落要因とされます
- インフレ率 → 金需要に影響を与える要因の一つです
- ドル相場 → 金価格と逆相関関係にあるとされています
...
### ⚠️ 一般的なリスク管理の考え方
- ポジションサイズ: 一般的に資金の2〜5%が推奨されています
- 週末リスク: 金曜夕方までに50%利確を検討する考え方もあります
- 情報収集: 毎日2回（朝・夕）のニュースチェックが推奨されています
...
st.markdown("---")
st.markdown("""
<div style="background: rgba(30, 30, 30, 0.6); padding: 2rem; border-radius: 12px; margin-top: 2rem; border: 1px solid rgba(0, 170, 255, 0.3);">
    <h3 style="color: #ff6600; margin-top: 0;">⚠️ 免責事項・利用規約</h3>
    
    <h4 style="color: #00aaff; margin-top: 1.5rem; font-size: 1rem;">1. サービスの性質</h4>
    <p style="font-size: 0.85rem; line-height: 1.7; color: #c0d8ff;">
    本アプリは、金相場（XAUUSD）のテクニカル分析を学習するための教育ツールです。
    表示される情報は一般的なテクニカル分析の教科書的な内容を現在の価格に当てはめた「分析例」であり、
    投資助言・売買推奨ではありません。
    </p>
    
    <h4 style="color: #00aaff; margin-top: 1.2rem; font-size: 1rem;">2. 投資判断の責任</h4>
    <p style="font-size: 0.85rem; line-height: 1.7; color: #c0d8ff;">
    実際の投資判断は、必ずご自身の責任と判断で行ってください。
    本アプリの情報を参考にした取引による損失について、運営者は一切の責任を負いません。
    </p>
    
    <h4 style="color: #00aaff; margin-top: 1.2rem; font-size: 1rem;">3. 金融商品取引業について</h4>
    <p style="font-size: 0.85rem; line-height: 1.7; color: #c0d8ff;">
    本アプリの運営者は金融商品取引業の登録を行っておりません。
    本アプリは教育目的のみで提供されており、投資助言業務は行っておりません。
    </p>
    
    <h4 style="color: #00aaff; margin-top: 1.2rem; font-size: 1rem;">4. リスクの認識</h4>
    <p style="font-size: 0.85rem; line-height: 1.7; color: #c0d8ff;">
    金融商品取引にはリスクが伴います。投資元本を割り込む可能性があります。
    取引を行う際は、リスクを十分に理解した上で、余裕資金で行ってください。
    </p>
    
    <h4 style="color: #00aaff; margin-top: 1.2rem; font-size: 1rem;">5. 情報の正確性</h4>
    <p style="font-size: 0.85rem; line-height: 1.7; color: #c0d8ff;">
    本アプリで提供される情報の正確性・完全性・有用性については保証いたしません。
    情報は予告なく変更される場合があります。
    </p>
    
    <p style="margin-top: 1.5rem; font-size: 0.8rem; color: #8b9dc3; line-height: 1.6;">
    最終更新日: 2026年3月8日<br>
    本アプリを利用することで、上記の免責事項に同意したものとみなされます。
    </p>
</div>
""", unsafe_allow_html=True)
